...
> 现在有了AI, 之前的maple-latex工具就完全不需要了, 所以最近下载了Maple 2023. 而且现在写代码完全可以交给AI, 我们只需要能够把问题描述清楚, 能看懂AI写的代码即可. 大多数时候, 写代码遇到的问题是不知道用什么函数, 而这是AI擅长的. 我们只需要在AI给的代码基础上就行修改.
...
a := 3; 
print(a); 
printf(`a = %f `, a); # 注意 `printf` 函数从行首开始打印, 并且默认不换行
printf(`a = %g \n`, a); # `\n`是换行符
printf(`b = %s`, "string")
```
...
> 所以无法直接比较两个矩阵是否相等. 可以两个矩阵相减, 然后计算差矩阵的元素平方和是否为0.
> ```javascript
...
### 绘图
Maple提供了针对各种函数类型的绘制函数, 具体可见`plots`包
...
implicitplot(x^2 + y^2 = 4, x = -3 .. 3, y = -3 .. 3, 
             filled = true, 
             coloring = ["Red", "Green"], # 内部红色，外部绿色
             style = surface);
```
![implicitplot](images/implt.png)
对于参数方程, 绘制方法为
```javascript
plot([cos(t), sin(t), t = 0..2 Pi])
```
![paraplt](images/paraplt.png)
另外, 极坐标
```javascript
polarplot(1 + cos(t), t = 0 .. 2*Pi, filled = true, color = "Red", background = "Green")
```
![polarplot](images/polarplt.png)
#### 图像叠加
Maple中可以使用`display`函数将多个函数绘制在同一个坐标轴中.
```javascript
p1 := plot(cos(x), x=0..Pi):
...
### 图像导出
Maple绘制的图形可以导出为图片格式(jpg, png等)或者矢量格式(svg, pdf), 但是在实际使用中, 会遇到有一些问题. 
下面给出一个示例来说明如何导出高清以及无损图片. 
```javascript
# 1. 环境初始化与函数定义
with(plots):
wxy := -8 * (50*x^2 + 20*x*y - 16*y^2 - 225) / (10*x^2 + 4*x*y + 4*y^2 + 45)^2:

# 2. 生成密度图 (P1)
R := 15:
P1 := densityplot(wxy, x = -R .. R, y = -R .. R, 
                  axes = boxed, 
                  grid = [100, 100], 
                  style = patchnogrid, 
                  colorstyle = HUE,
...
# 3. 生成截面对比图 (P2)
p1 := plot(subs(x = -2.5, wxy), y = -R .. R, 
           legend = typeset(x = -2.5), 
           color = "red", 
           legendstyle = [location = "left"]):
p2 := plot(subs(x = -1.5, wxy), y = -R .. R, 
           legend = typeset(x = -1.5), 
           color = "blue", 
           linestyle = dashdot):
p3 := plot(subs(x = -0.5, wxy), y = -R .. R, 
           legend = typeset(x = -0.5), 
           color = "black", 
           linestyle = dash):

P2 := display([p1, p2, p3], 
              size = [500, 300], 
              labels = [typeset(y), typeset('w')], 
              labelfont = ["Sim", 20], 
              axesfont = ["Sim", 20], 
...
# 导出 P2 (SVG格式), 这里也可以用pdf, 但是导出来的结果有问题 (图像过于复杂)
plotsetup(svg, plotoutput = cat(path, "lump2yu-xy")):
P2;
plotsetup(default):
...
> `typeset`函数, 可以用来给图形添加数学注释, 公式等, 支持tex公式.

> `.svg`类型的文件是标记型图形, 具有无损放缩的效果, 可以用浏览器打开. 支持编辑(需要借助于一些svg编辑工具), 也可以导出为pdf文件, 用于latex中. 在实际使用中, 由于Maple导出的svg图形, 部分需要修改, 所以我借助于Google的AI Studio写了个svg的在线编辑器[SVG Studio](https://jiandandaoxingfu.github.io/SVG-Studio/), 可以对svg进行编辑.
...
{{< download url="files/2026/2026-03-20-Lesson 3.mw" text="Lesson 3: 实战: 双线性方法求解lump解" >}}

{{< download url="files/2026/2026-03-27-Lesson 4.mw" text="Lesson 4: 实战: 双线性方法求解lump解" >}}

{{< download url="files/2026/BKP.pdf" text="Lesson 4: 练习" >}}
